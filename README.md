...
**Telefono:** 340 292 9928 · **Email:** vibrissecodine@gmail.com
...
Le immagini della galleria si trovano in [`assets/gallery/`](assets/gallery).
Per aggiungerne di nuove:

1. Copia la foto nella cartella `assets/gallery/` (formato consigliato:
   `.jpg` o `.png`, orientamento verticale);
2. In `index.html`, nella sezione `#galleria`, duplica un blocco `<figure
   class="gallery-slide">…</figure>` esistente e aggiorna `src`, `alt` e
   `figcaption`;
3. Lo slider si aggiorna automaticamente: nessuna configurazione aggiuntiva
   richiesta.
...
│   ├── watermark.png         # firma "manu1394©" nel footer
...
3. In *Source*, seleziona il branch principale (es. `main`) e la cartella
   `/ (root)`.
4. Salva: il sito sarà online in pochi minuti all'indirizzo
   `https://<utente>.github.io/<nome-repository>/`.

Nessuna build, nessuna dipendenza da installare: è puro HTML, CSS e
JavaScript.
